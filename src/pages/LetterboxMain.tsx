...
function Step3({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  ...
  const hasVideo = !!(draft.youtubeLink || draft.attachmentName)
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_440px] gap-14 items-start animate-fade-up">
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between pt-2">
          ...
          <Button size="default" onClick={onNext} disabled={!draft.lesson.trim()}>
            Continue →
          </Button>
        </div>
      </div>
...
export function LetterboxMain() {
  ...
  const pageSubtitle =
    flow === 'transit'  ? 'Drafts, letters in transit, and replies received.' :
